...
pub async fn start_download(
    ...
    tracing::info!(
        "Comando start_download invocado. URL: {}, Destino: {}",
        url,
        destination
    );
    let dest_path = PathBuf::from(destination);
    let path = download_service
        .start_download(&url, dest_path, app)
        .await?;
    Ok(path.to_string_lossy().to_string())
}

#[tauri::command]
pub async fn cancel_download(download_service: State<'_, DownloadService>) -> AppResult<()> {
...
pub async fn verify_image(
    ...
    verification_service
        .verify_sha256(&path, expected_hash.as_deref(), app)
        .await
}
